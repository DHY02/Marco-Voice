...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
  echo "Run train. We only support llm training for now. If your want to train from scratch, please use conf/cosyvoice.fromscratch.yaml"
  if [ $train_engine == 'deepspeed' ]; then
    echo "Notice deepspeed has its own optimizer config. Modify conf/ds_stage2.json if necessary"
  fi
  # cat data/{train-clean-100,train-clean-360,train-other-500}/parquet/data.list > data/train.data.list
  # cat data/{dev-clean,dev-other}/parquet/data.list > data/dev.data.list
  for dir in train-clean-100 train-clean-360 train-other-500; do
    cat "data/$dir/parquet/data.list"
  done > data/train.data.list

  # For dev data
  for dir in dev-clean dev-other; do
      cat "data/$dir/parquet/data.list"
  done > data/dev.data.list
