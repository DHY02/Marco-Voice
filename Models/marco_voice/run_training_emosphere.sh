...
custom_data_dir=Marco-Voice/test_set/   # directory containing train, dev, test splits
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
  ...
  for split in train; do
    tools/extract_embedding_emosphere.py --dir ${processed_data_dir}/ \
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
  ...
  for split in train; do
    ...
    tools/make_parquet_list_emosphere.py --num_utts_per_parquet 1000 \
      --num_processes 10 \
