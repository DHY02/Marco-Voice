...
def job(utt_list, parquet_file, utt2parquet_file, spk2parquet_file):
    ...
    # emotionembedding_list = [utt2emotion_embedding[utt] for utt in utt_list]  
    speech_token_list = [utt2speech_token[utt] for utt in utt_list]
    for utt in utt_list:
        emotionembedding_list.append(utt2emotion_embedding[utt])

    # save to  parquet, utt2parquet_file, spk2parquet_file
    df = pd.DataFrame()
    df['utt'] = utt_list
    df['wav'] = wav_list
    ...
    df['emotion_embedding'] = emotionembedding_list  #  emotion_embedding
...
if __name__ == "__main__":
    ...
    utt2emotion_embedding = torch.load('{}/utt2emotion_embedding.pt'.format(args.src_dir))  
