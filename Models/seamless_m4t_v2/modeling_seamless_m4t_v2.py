...
class SeamlessM4Tv2ScaledWordEmbedding(nn.Embedding):
    ...
    def forward(self, input_ids: torch.Tensor):
        ...
        print("Before conversion - input_ids dtype:", input_ids.dtype)  

        if not input_ids.dtype in (torch.int64, torch.int32):
...
class SeamlessM4Tv2SinusoidalPositionalEmbedding(nn.Module):
    ...
    def create_position_ids_from_inputs_embeds(self, inputs_embeds, past_key_values_length):
        ...
        position_ids = torch.arange(
            self.padding_idx + 1, sequence_length + self.padding_idx + 1, dtype=torch.int32, device=inputs_embeds.device
        )
...
class SeamlessM4Tv2Attention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            tgt_len = attention_scores.size(2)  # 1024
            src_len = attention_scores.size(3) 
            if attention_mask.size(-1) > src_len:
                attention_mask = attention_mask[..., :src_len]
                attention_mask = attention_mask[:,:,:attention_scores.shape[2], :]

            if attention_mask.size(1) != attention_scores.size(1):
                attention_mask = attention_mask.expand(-1, attention_scores.size(1), -1, -1)
...
class SeamlessM4Tv2Decoder(SeamlessM4Tv2PreTrainedModel):
    ...
    def forward(
        ...
        attention_mask = _prepare_4d_causal_attention_mask(
            attention_mask[:, inputs_embeds.shape[-1]], input_shape, inputs_embeds, past_key_values_length
        )
...
class SeamlessM4Tv2TextToUnitDecoder(SeamlessM4Tv2PreTrainedModel):
    ...
    def forward(
        ...
        char_hidden_states = char_positions + char_hidden_states #  + self.embed_char(char_input_ids) * self.embed_scale # 10934*7, 5052*1024
...
class SeamlessM4Tv2CodeHifiGanWithEmotion(SeamlessM4Tv2CodeHifiGan):
    def __init__(self, config):
        super().__init__(config)

        # 添加 emotion embedding 和线性层 emotion_embedding [192]
    ...
    def forward(
        ...
        """
        hidden_states = self.unit_embedding(input_ids).transpose(1, 2)
        spkr = self.speaker_embedding(spkr_id).unsqueeze(1).transpose(1, 2)
        lang = self.language_embedding(lang_id).unsqueeze(1).transpose(1, 2)

        # emotion = self.emotion_embedding(emotion_embedding).transpose(1, 2)
        emotion = self.emotion_proj(emotion_embedding).unsqueeze(1).transpose(1, 2)  
        spkr += emotion
...
class SeamlessM4Tv2CodeHifiGanWithEmoSphere(SeamlessM4Tv2CodeHifiGan):
    def __init__(self, config):
        super().__init__(config)
        # self.Lort_losss = False
        # self.cross_loss = False
        self.emo_VAD_inten_proj = nn.Linear(1, config.emotion_embed_dim, bias=True)
        self.emosty_layer_norm = nn.LayerNorm(config.emotion_embed_dim)
        ...
        self.emo_proj = nn.Linear(config.spkr_embed_dim, config.spkr_embed_dim, bias=True)
        ...
        self.emo_mlp = torch.nn.Sequential(
            torch.nn.Linear(config.emotion_embed_dim, 1024),
            Mish(),
            torch.nn.Linear(1024, config.spkr_embed_dim),
        )
    ...
    def forward(
        ...
        intens_embed = self.emo_VAD_inten_proj(low_level_emotion_embedding[:, 0:1])
        # style_vector=style_vector.squeeze(1) 
        ele_embed = 0
        elevation = low_level_emotion_embedding[:, 1:2]
        ...
        azi_embed = 0
        azimuth = low_level_emotion_embedding[:, 2:3]   
        ...
        # emotion = self.emotion_embedding(emotion_embedding).transpose(1, 2)
        emotion = self.emotion_proj(emo_all_emb).unsqueeze(2)  
...
class SeamlessM4Tv2TextToUnitForConditionalGenerationWithEmotionEmbedding(SeamlessM4Tv2TextToUnitForConditionalGeneration):
    def __init__(self, config: SeamlessM4Tv2Config, embed_tokens_decoder: Optional[nn.Embedding] = None):
        ...
        self.emotion_embedding_proj = nn.Linear(192, config.hidden_size)
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        emotion_embedding: Optional[torch.FloatTensor] = None,  
        **kwargs,
    ) -> Union[Seq2SeqLMOutput, Tuple[torch.FloatTensor]]:
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict

        # process emotion_embedding
        if emotion_embedding is not None:
            emotion_embedding = self.emotion_embedding_proj(emotion_embedding) 
            emotion_embedding = emotion_embedding.unsqueeze(1).repeat(1, inputs_embeds.size(1), 1) 
        ...
        if labels is not None:
            ...
            labels = labels.to(lm_logits.device)
            masked_lm_loss = loss_fct(lm_logits.view(-1, self.config.vocab_size).to(torch.float32)[:labels.view(-1).shape[0], :], labels.view(-1).to(torch.long))
...
#             emotion_embedding = self.emotion_embedding_proj(emotion_embedding) 
#             emotion_embedding = emotion_embedding.unsqueeze(1).repeat(1, inputs_embeds.size(1), 1) 
#             # print("inputs_embeds:", inputs_embeds.shape, emotion_embedding.shape)
#             inputs_embeds = inputs_embeds + emotion_embedding 
...
class SeamlessM4Tv2TextToUnitForConditionalGenerationWithEmoSphere(SeamlessM4Tv2TextToUnitForConditionalGeneration):
    def __init__(self, config: SeamlessM4Tv2Config, embed_tokens_decoder: Optional[nn.Embedding] = None):
        super().__init__(config, embed_tokens_decoder)
    # def __init__(
    #     self,
    ...
    # ):
        ...
        self.sty_proj = nn.Linear(config.hidden_size, config.hidden_size//2, bias=True)
        ...
        self.emo_proj = nn.Linear(config.hidden_size, config.hidden_size, bias=True)

        self.emo_mlp = torch.nn.Sequential(
            torch.nn.Linear(config.hidden_size, 1024),
            Mish(),
            torch.nn.Linear(1024, config.hidden_size//2),
        )
    ...
    #     return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        ...
        ele_embed = elevation_embed + ele_embed
        azi_embed = 0
        azimuth = low_level_emotion_embedding[:, 2:3]   #提取仰角和方位
        azimuth_index = torch.bucketize(azimuth, self.azimuth_bins)
        azimuth_index = azimuth_index.squeeze(1)
        azimuth_embed = self.azimuth_emb(azimuth_index)
        azi_embed = azimuth_embed + azi_embed # [11, 96]
...
class SeamlessM4Tv2ForTextToSpeechWithEmotionEmbedding(SeamlessM4Tv2ForTextToSpeech):
    def __init__(self, config: SeamlessM4Tv2Config, SeamlessM4Tv2ForTextToSpeechWithEmotionEmbedding=None):
        super().__init__(config)

        # Replace t2u_model with custom version if provided
        if SeamlessM4Tv2TextToUnitForConditionalGenerationWithEmotionEmbedding is not None:
            self.t2u_model = SeamlessM4Tv2TextToUnitForConditionalGenerationWithEmotionEmbedding(config)

        # Additional layer to process the extra input
        self.additional_projection = nn.Linear(config.hidden_size, config.hidden_size)

        # T2U loss weight
        self.t2u_loss_weight = 1.0  # Can be configured
    ...
    def forward(
        ...
        decoder_inputs_embeds: Optional[torch.FloatTensor] = None,
        labels: Optional[torch.LongTensor] = None,
        t2u_labels: Optional[torch.LongTensor] = None,  # New: labels for T2U model
        use_cache: Optional[bool] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        ...
        if emotion_embedding is not None:
            projected_features = self.additional_projection(emotion_embedding)
        else:
        ...
        if encoder_outputs is None:
            encoder_outputs = self.text_encoder(
                ...
                inputs_embeds=inputs_embeds,
                output_attentions=output_attentions,
                output_hidden_states=True,  # Force output hidden states for T2U
                return_dict=return_dict,
            )
        elif return_dict and not isinstance(encoder_outputs, BaseModelOutput):
            encoder_outputs = BaseModelOutput(
        ...
        # Calculate T2U loss if labels provided
        t2u_loss = None
        if t2u_labels is not None:
            # Get decoder hidden states as input to T2U model
            decoder_hidden_states = decoder_outputs.hidden_states[-1]

            # Prepare T2U inputs
            t2u_input_embeds = decoder_hidden_states
            t2u_attention_mask = attention_mask

            # Forward pass through T2U model
            t2u_outputs = self.t2u_model(
                inputs_embeds=t2u_input_embeds,
                attention_mask=t2u_attention_mask,
                labels=t2u_labels,
                emotion_embedding=projected_features,
                return_dict=True
            )
            t2u_loss = t2u_outputs.loss * self.t2u_loss_weight

        # Combine losses
        total_loss = masked_lm_loss
        if t2u_loss is not None:
            total_loss = masked_lm_loss + t2u_loss if masked_lm_loss is not None else t2u_loss

        if not return_dict:
            outputs = decoder_outputs + encoder_outputs
            output = (lm_logits,) + outputs[1:]
            if t2u_loss is not None:
                output = (total_loss,) + output + (t2u_loss,)
            return output
        ...
        return Seq2SeqLMOutput(
            ...
            encoder_hidden_states=encoder_outputs.hidden_states,
            encoder_attentions=encoder_outputs.attentions,
            t2u_loss=t2u_loss,  # Add T2U specific outputs
            t2u_logits=getattr(t2u_outputs, 'logits', None) if t2u_labels else None
        )
    ...
    def generate(
        ...
        tgt_lang: Optional[str] = None,
        speaker_id: Optional[int] = 0,
        emotion_embedding: Optional[torch.FloatTensor] = None,  # Added additional features
        **kwargs,
    ) -> Union[torch.Tensor, SeamlessM4Tv2GenerationOutput]:
        # Process additional features if provided
        if emotion_embedding is not None:
        ...
        if tgt_lang is None:
            ...
            for key in ["text_decoder_lang_to_code_id", "t2u_lang_code_to_id", "vocoder_lang_code_to_id"]:
                ...
                if lang_code_to_id is None:
                    raise ValueError(
                        f"This model generation config doesn't have a `{key}` key which maps the target language "
                        f"to the right token id. Make sure to load the right generation config."
                    )
                elif tgt_lang not in lang_code_to_id:
                    raise ValueError(
                        f"`tgt_lang={tgt_lang}` is not supported by this model. "
                        f"Please specify a `tgt_lang` in {','.join(lang_code_to_id.keys())}."
                    )
        ...
        # Compute character-level inputs
        t2u_subwords = self._indices_to_subwords(t2u_input_ids)
        t2u_char_count_per_id = self._count_character_length_in_subword(
            t2u_input_ids, t2u_subwords, pad_token_id=pad_token_id
        )
        ...
        # Second pass through custom t2u model with additional features
        t2u_output = self.t2u_model(
            inputs_embeds=t2u_input_embeds,
            char_input_ids=t2u_char_input_ids,
            char_count_per_id=t2u_char_count_per_id,
            emotion_embedding=projected_features,  # Pass additional features to custom t2u model
            **kwargs_speech,
        )
        ...
        # Generate speech with custom vocoder
        vocoder_tgt_lang_id = self.generation_config.vocoder_lang_code_to_id.get(tgt_lang)
        vocoder_tgt_lang_id = torch.tensor([[vocoder_tgt_lang_id]] * len(unit_ids)).to(self.device)
        speaker_id = torch.tensor([[speaker_id]] * len(unit_ids)).to(self.device)

        # Pass additional features to custom vocoder if needed
        waveform, waveform_lengths = self.vocoder(
            input_ids=unit_ids,
            speaker_id=speaker_id,
            lang_id=vocoder_tgt_lang_id,
            emotion_embedding=projected_features  # Optional: pass to vocoder if needed
        )
...
class SeamlessM4Tv2ForTextToSpeechWithEmotion(SeamlessM4Tv2ForTextToSpeech):
    def __init__(self, config: SeamlessM4Tv2Config):
        super().__init__(config)

        # Replace the original t2u_model 
        self.t2u_model = SeamlessM4Tv2TextToUnitForConditionalGenerationWithEmotionEmbedding(config)
        self.speech_encoder = SeamlessM4Tv2SpeechEncoder(config)

        # Freeze all parameters except those in t2u_model
        for name, param in self.named_parameters():
            if not name.startswith('t2u_model.'):
    ...
    def forward(
        ...
        return_dict: Optional[bool] = None,
        emotion_embedding: Optional[torch.FloatTensor] = None,
        char_input_ids: Optional[torch.FloatTensor] = None,
        char_count_per_id: Optional[torch.FloatTensor] = None
    ) -> Union[Seq2SeqLMOutput, Tuple[torch.FloatTensor]]:
        # print("t2u_labels:", t2u_labels)
        # We only care about the encoder outputs for the t2u model
        if encoder_outputs is None:
            encoder_outputs = self.text_encoder(
                input_ids=input_ids,
                attention_mask=attention_mask,
                inputs_embeds=inputs_embeds,
                output_attentions=output_attentions,
                output_hidden_states=True,  # Need hidden states for t2u
                return_dict=return_dict,
            )
        elif return_dict and not isinstance(encoder_outputs, BaseModelOutput):
            encoder_outputs = BaseModelOutput(
        ...
        # Get decoder outputs (we need the hidden states for t2u)
        decoder_outputs = self.text_decoder(
            input_ids=decoder_input_ids,
            attention_mask=decoder_attention_mask,
            encoder_hidden_states=encoder_outputs[0],
        ...
        # Only compute t2u loss if labels are provided
        t2u_loss = None
        if t2u_labels is not None:
            # Get decoder hidden states as input to T2U model
            t2u_input_embeds = decoder_outputs.hidden_states[-1]

            # Forward pass through custom t2u model
            t2u_outputs = self.t2u_model(
                inputs_embeds=t2u_input_embeds,
                attention_mask=attention_mask,
                labels=t2u_labels,
            ...
            t2u_loss = t2u_outputs.loss
        if not return_dict:
            return t2u_loss
            # outputs = decoder_outputs + encoder_outputs
            # output = (None,) + outputs[1:]  # No lm_logits in output since we're not using them

            # if t2u_loss is not None:
            #     output = (t2u_loss,) + output
            # return output

        return Seq2SeqLMOutput(
            loss=t2u_loss,  # Only return t2u loss
    ...
    def generate(
        ...
        **kwargs,
        ...
        # Second pass through custom t2u model with emotion embedding
        t2u_output = self.t2u_model(
            inputs_embeds=t2u_input_embeds,
            char_input_ids=t2u_char_input_ids,
            char_count_per_id=t2u_char_count_per_id,
...
__all__ = [
    ...
    "SeamlessM4Tv2ForTextToSpeechWithEmotion",
    "SeamlessM4Tv2ForSpeechToSpeech",
    "SeamlessM4Tv2ForTextToText",
    "SeamlessM4Tv2ForSpeechToText",
    "SeamlessM4Tv2Model",
    "SeamlessM4Tv2PreTrainedModel",
    "SeamlessM4Tv2CodeHifiGanWithEmotion",
    "SeamlessM4Tv2CodeHifiGanWithEmoSphere",
    "SeamlessM4Tv2TextToUnitForConditionalGenerationWithEmotionEmbedding",
    "SeamlessM4v2TTextToUnitForConditionalGenerationWithEmoSphere",
