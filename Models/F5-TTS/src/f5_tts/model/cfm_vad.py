...
class CFM(nn.Module):
    def __init__(
        ...
        # emotion embedding 
        self.emo_VAD_inten_proj = nn.Linear(1, 2 * num_channels, bias=True)
        self.emosty_layer_norm = nn.LayerNorm(2 * num_channels)

        self.sty_proj = nn.Linear(num_channels, num_channels, bias=True)

        self.azimuth_bins = nn.Parameter(torch.linspace(-np.pi/2, np.pi, 4), requires_grad=False)
        self.azimuth_emb = torch.nn.Embedding(4, num_channels // 2)
        self.elevation_bins = nn.Parameter(torch.linspace(np.pi/2, np.pi, 2), requires_grad=False)
        self.elevation_emb = torch.nn.Embedding(2, num_channels // 2)

        # self.emo_proj = nn.Linear(192, num_channels, bias=True)

        self.emo_mlp = torch.nn.Sequential(
            torch.nn.Linear(192, 1024),
            Mish(),
            torch.nn.Linear(1024, num_channels),
        )
        self.emotion_embedding_projector = torch.nn.Linear(200, num_channels)
    ...
    def sample(
        ...
        emotion_embedding: float["b d"],
        low_level_emo_embedding: float["b d"], 
        ...
        *,
        ...
        embedding = F.normalize(emotion_embedding, dim=1)
        emos_proj_embed = self.emo_mlp(embedding)
        ...
        emotion_embedding = self.emotion_embedding_projector(emotion_embedding)
        emotion_expanded = emotion_embedding.unsqueeze(1).repeat(1, cond.size(1), 1)  # [bn, length, dim]
        cond += emotion_expanded
    ...
    def forward(
        ...
        *,
        ...
        emosty_embed = self.emosty_layer_norm(emotion_embedding)
        emo_all_emb = (intens_embed + emosty_embed) # torch.Size([11, 384])
        emotion_embedding = self.emotion_embedding_projector(emotion_embedding) # (9x200 and 192x100). (9x200 and 192x200)
        emotion_expanded = emotion_embedding.unsqueeze(1).repeat(1, inp.size(1), 1)  # [bn, length, dim]
        inp += emotion_expanded
        if inp.ndim == 2:
